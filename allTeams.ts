...
const jsonsInDir = fs.readdirSync('./bbl_json').filter(file => path.extname(file) === '.json');
let gameNo = 1; 
const myConsole = new console.Console(fs.createWriteStream('./output-allteams.txt'));
let pass = 0;
let fail = 0;
...
sortedJsonFilesInDir.forEach((file: any) => {
  const fileData = fs.readFileSync(path.join('./bbl_json', file.toString() + '.json'));
